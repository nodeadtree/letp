...
# Introduction
This paper describes a Python package, letp (lightweight extensible testing pipeline), used to
provide structure to a data analysis project, or set of Machine Learning experiments. Many exper-
iments can be broken up into a couple common procedures that frequently get reimplemented or
simply copied from script to script. This particular piece of software exists to fill some common
gaps in an easy, open and interoperable fashion. This paper describes some of the common machine
learning scenarios, examines procedures, and tries to highlight the common features, how the letp
package can fulfill these roles, and why someone might choose to include letp into their testing
toolkit. In addition, there will be a simple walk through the development of a set of experiments,
and reference for the letp software. Primarily, the goal of this software package is to reduce the
amount of redundant code involved in the evaluation of a set of algorithms and allow for an easy interchange of
components.

# Background
Machine learning experiments typically consist of several common features, reading data, per-
forming analysis with some kind of algorithm, and taking measurements of the algorithm’s perfor-
mance. When performing some kind of model evaluation, one can reasonably expect
the same sort of behaviors and interactions. Regardless of what algorithms you use, the data will
need to be managed. Regardless of what data is used, there will consistently be something reading
that data, and a need to perform analysis of the algorithm’s results. In this entire process, there are several similar components, which can be enumerated thusly. There is a dataset that is being examined, there are algorithms that perform analysis on the dataset, and then there is an analysis of these measurements to make comparisons

This structure is consistently seen across various pieces of literature, from Hastie's *Elements of Statistical Learning*, from bioinformatics papers, to machine learning toolkits like Scikit-Learn and Keras. In each of these, the following pattern is present.
![ML Data Graph](https://user-images.githubusercontent.com/30419327/61099565-a327ac00-a417-11e9-979a-3cfc8fcac0ec.png)

The Raw Data is data that is to be used as input to some algorithm, the algorithm is whatever machine learning model the user would like to inspect, the measurements are recordings of states or outputs of the algorithm, and the comparison performs calculations of these states or outputs.

Once we enumerate these common processes, we can proceed to the design of the software. The first common process is the data management. In any of the machine learning tasks, the user must collect data from a file, or from some simulator, and pass it along to a classifier. However, before this can be done, several things must occur, this consists of several smaller steps. Firstly, it makes the data accessible to the computer, either by downloading, collecting raw data, or by importing some file. This is a mandatory step for any data analysis process. There may be other components as well. Supposing the data contains null-entries or some other error, they must be corrected, and the integrity of the data would need to be verified. Additionally, there may be desired feature transformations that would occur on the data. Thankfully, each of these can occur during the reading of data, so they will be considered in the same family of functions, Data Reading and Filtration. The other type of processing that may need to happen between the input of raw data and input of processed data into the classifier, is partitioning. In a wide number of machine learning tasks, there is a need to separate data into training set, and a testing set. Additionally, there may be further complications to this procedure, through things like k-fold cross validation, or other data partitioning methodologies. This second type of data transformation is called Data Partitioning.

The next common component of these comparisons is to take the processed data, and collect measurements from an algorithm. For some feature extraction tests, this may involve recording the internal state of some algorithm. For classification tasks, this may involve recording the labels of data and the predictions made by the algorithm. For an unsupervised learning task, this may involve recording the inferred structure of the data. For a regression task, this may involve the collection of the regressor. Additional measurements that might be collected are things like training time, size of data, dimension of data, or size of the model. Each of these components are directly collected from the algorithm, and minimal computations are performed on them at this stage. There are two types of functions here, Model Training and Testing, and measurement collection.
...
The letp package is designed to fill the gaps shown in the previous diagram, and attempts to do so by defining three classes which handle these components. These classes are as follows.
* DataHandler
* Cycle
* Analyzer
...
The DataHandler is where the reading and filtration of data are handled. The reader/filter is defined as an argument during object instantiation, as is the partitioner. The reader/filter are functions that are used on raw data, to either trim faulty values, collect specific subsets, or perform some feature transform on the data. The partitioner is the function that works with clean data, and is responsible for breaking the data into testing or training segments. It can be used for other things, but this is the intended role.


### Cycle

The cycle is an object used for defining the model training and testing. It is responsible for feeding data into the model, and reporting measurements to the Analyzer. The behavior of the analyzer is defined by the step argument, which is responsible for taking the data from the data handler, and returning measurements to be used in computations by the Analyzer. Once a cycle is defined, its step function can be run with the Cycle's run() method. The analyses generated for each run are returned by the run() method.

### Analyzer

The analyzer is where measurements are reported, and where they have their meta-statistics computed. Every time a Cycle is run, the Analyzer is used to generate meta-analyses, report them to the cycle, and write them to a json file if the user desires.
