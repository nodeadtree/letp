...
# Test cases for the process_data function. By default, these tests use no
...
process_data_functions = [identity]
read_data_functions = [identity]
test_process_data_params = [(size_combos, identity, identity)]


@pytest.mark.parametrize("reader,processor", zip(process_data_functions,
                                                 process_data_functions))
def test_None_input_failure(reader, processor):
    with pytest.raises(Exception):
        mldp.process_data(None, reader, processor)
...
def test_no_input_failure(reader, processor):
    with pytest.raises(Exception):
        mldp.process_data(reader=reader, processor=processor)


@pytest.mark.parametrize("sizes,reader,processor", zip(size_col,
                                                       process_data_functions,
                                                       process_data_functions))
def test_process_data_for_numpy_arrays(sizes, reader, processor):
    for i in sizes:
        assert isinstance(mldp.process_data(generate_test_matrix(i),
...
@pytest.mark.parametrize("sizes,reader,processor", zip(size_col,
                                                       process_data_functions,
                                                       process_data_functions))
def test_process_data_for_numpy_array_dimension(sizes, reader, processor):
    print(sizes)
    for i in sizes:
        assert mldp.process_data(generate_test_matrix(i),
